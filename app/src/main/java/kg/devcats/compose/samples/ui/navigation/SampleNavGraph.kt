...
import kg.devcats.compose.samples.ui.chili_sample.PreviewPullToRefresh
import kg.devcats.compose.samples.ui.chili_sample.PreviewTooltip
import kg.devcats.compose.samples.ui.chili_sample.pin_lock.PreviewPinInput
...
fun SamplesNavGraph(navController: NavHostController = rememberNavController()) {
    ...
    NavHost(navController = navController, startDestination = Screens.MainScreen.toString()) {
        ...
        composable(Screens.TooltipScreen.toString()) {
            PreviewTooltip { navController.navigateUp() }
        }
        composable(Screens.PullToRefreshScreen.toString()) {
            PreviewPullToRefresh { navController.navigateUp() }
        }
    }
}